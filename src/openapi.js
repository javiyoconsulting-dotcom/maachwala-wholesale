...
const openapiDocument = {
  ...
  paths: {
    ...
    '/wholesale/updatesalesresponse': {
      post: postOperation('Update buyer allocations by sorting number', 'Allocations', { $ref: '#/components/schemas/UpdateSalesResponseRequest' }, { responseSchema: jsonObject })
    },
    '/wholesale/notsettledtransactions': {
      post: postOperation('Get buyer allocations that are not settled', 'Allocations', orgRequest, { responseSchema: { type: 'array', items: { $ref: '#/components/schemas/NotSettledTransaction' } } })
    },
    ...
    '/wholesale/getsuppliers': {
      post: postOperation('Get supplier names and phone numbers', 'Master Data', orgRequest, { responseSchema: { type: 'array', items: { $ref: '#/components/schemas/Supplier' } } })
    },
    '/wholesale/createsuppliers': {
      post: postOperation('Create suppliers with unique phone numbers', 'Master Data', { $ref: '#/components/schemas/CreateSuppliersRequest' }, { successStatus: 201, responseSchema: jsonObject })
    },
  ...
  components: {
    schemas: {
      ...
      SalesSummaryGroup: { type: 'object', properties: { supplier: { type: 'string' }, product: { type: 'string' }, totalSalesQuantity: { type: 'number' }, averageUnitPrice: { type: 'number', description: 'Total cost divided by discounted total weight.' }, cashCollection: { type: 'number', description: 'Total billable amount for cash and debit transactions.' }, creditCollection: { type: 'number', description: 'Total billable amount for credit transactions.' }, totalCost: { type: 'number', description: 'Sum of cashCollection and creditCollection.' }, weightDiscount: { type: 'number', description: 'Discounted total weight used to calculate averageUnitPrice.' }, salesRecords: { type: 'array', items: jsonObject } } },
      SalesSummary: { type: 'object', properties: { orgid: { $ref: '#/components/schemas/OrgId' }, date: { type: 'string', format: 'date' }, discountWeight: { type: 'number' }, totalCashCollection: { type: 'number', description: 'Sum of cashCollection across every supplier-product group.' }, totalCredit: { type: 'number', description: 'Sum of creditCollection across every supplier-product group.' }, groupCount: { type: 'integer' }, groups: { type: 'array', items: { $ref: '#/components/schemas/SalesSummaryGroup' } }, invalidRecordCount: { type: 'integer' }, invalidRecords: { type: 'array', items: jsonObject }, generatedAt: { type: 'string', format: 'date-time' } } },
      Associate: { type: 'object', required: ['name', 'phone'], properties: { name: { type: 'string' }, phone: { oneOf: [{ type: 'string' }, { type: 'integer' }] } } },
      CustomerCreateInput: { type: 'object', required: ['name'], properties: { name: { type: 'string', maxLength: 200 }, phone: { oneOf: [{ type: 'string', pattern: '^\\d{6,15}$' }, { type: 'integer' }], nullable: true, description: 'Optional. Numeric strings are converted to a number; missing, null, or blank values are stored as null.' } } },
      CreateCustomersRequest: { type: 'object', required: ['orgid', 'customers'], properties: { orgid: { $ref: '#/components/schemas/OrgId' }, customers: { type: 'array', items: { $ref: '#/components/schemas/CustomerCreateInput' } } } },
      ...
      CreatePurchaseRequest: { type: 'object', required: ['orgid'], properties: { orgid: { $ref: '#/components/schemas/OrgId' }, purchaseDate: { type: 'string', format: 'date', nullable: true }, totalCost: { type: 'number', nullable: true }, currency: { type: 'string', nullable: true, example: 'INR' }, products: { type: 'array', nullable: true, items: { $ref: '#/components/schemas/PurchaseProduct' } }, notes: { type: 'string', nullable: true } } },
      ...
      UpdateSalesResponseRequest: { type: 'object', required: ['orgid', 'sortingnumber', 'buyerunitprice', 'buyerquantity', 'buyerweightdiscount'], properties: { orgid: { $ref: '#/components/schemas/OrgId' }, sortingnumber: { oneOf: [{ type: 'string', pattern: '^\\d+$' }, { type: 'integer', format: 'int64' }] }, buyerunitprice: { type: 'number', minimum: 0 }, buyerquantity: { type: 'number', minimum: 0 }, buyerweightdiscount: { type: 'number', minimum: 0 } } },
      NotSettledTransaction: { type: 'object', properties: { actualWeight: { type: 'number', nullable: true }, maximumPrice: { type: 'number', nullable: true }, minimumPrice: { type: 'number', nullable: true }, buyerWeight: { type: 'number', nullable: true }, buyerPrice: { type: 'number', nullable: true }, buyerWeightDiscount: { type: 'number', nullable: true }, totalSalesAmount: { type: 'number', nullable: true }, sortingNumber: { type: 'number', nullable: true }, productDescription: { type: 'string', nullable: true }, sizeDescription: { type: 'string', nullable: true }, purchaseDate: { type: 'string', format: 'date', nullable: true }, purchaseNumber: { type: 'number', nullable: true }, totalCost: { type: 'number', nullable: true } } },
